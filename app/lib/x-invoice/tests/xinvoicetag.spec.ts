...
describe('XInvoice tag test suite', () => {
  ...
  it('should add a space between attributes if more than one attribute is passed', () => {
    const tag = new XInvoiceNumberTag<{ format: string; other: string }>(380, {
      attributes: { format: 'YYYY-MM-DD', other: 'other' },
    });
    expect(tag.toXMLTag('StartDate')).toEqual(
      '<cbc:StartDate format="YYYY-MM-DD" other="other">380</cbc:StartDate>'
    );
  });
});
